...
Los modelos elegidos fueron: Modelo 1 con lineal, Modelo 3 con árboles de regresión. Así entonces, se vuelven a correr los modelos finales, con todos los datos desde el año 2012 hasta el año 2017 para realizar las predicciones al primer semestre del año 2018.
...
pred3<- predict(mod_Final,newdata = db2018[,-1])
db2018$Fecha <- as.factor(db2018$Fecha)
pred3<- cbind.data.frame("Fecha"=db2018$Fecha,"prediccion" =pred3 )
...
predl<- cbind.data.frame("Fecha"=db2018$Fecha,"prediccion" =predl )
predl %>%kbl() %>%  kable_styling(font_size = 12)%>% kableExtra::scroll_box(width = "30%", height = "190px")
```
```{r eval=F}
library("writexl")
