...
Se dividen los datos en train con los años de 2012 a 2016 y se realiza test con los datos del año 2017. A continuación se lista el nombre de los festivos nacionales y las fechas especiales no festivas respectivamente:  
...
datos$Unidades<-as.integer(datos$Unidades)
datos<- datos[,-c(8:30)]
datos$observance <- ifelse(datos$Fecha %in% observance$date ,1,0) %>% factor()
train<-filter(datos, year<= 2016)
test <-filter(datos, year > 2016 )
cooksd <- cooks.distance(mod0)
influential <- na.omit(as.numeric(names(cooksd)[(cooksd > (4/nrow(train)))]))
...
# Modelo red neuronal
#library(neuralnet)
#mod3 <- neuralnet(Unidades ~ ., data=train, hidden=c(3), threshold=0.01)
```
```{r}
library(nnet)
library(caret)

model.nn <- train(Unidades ~ .,data = train2, method = "nnet")
```
```{r}
print(model.nn)
```
```{r}
pred3 <- predict(model.nn, test[,-2])
accuracy <- sum(pred3 == test$Unidades)/length(test$Unidades)
print(accuracy)
```
```{r}
...
Se creó una aplicación shiny con el propósito de que una persona pueda hacer uso del modelo predictivo ingresando una respectiva fecha, en el siguiente enlace - - - - - se encuentra disponible un video explicativo sobre cómo usar dicha información aplicación.
