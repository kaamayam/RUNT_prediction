...
ggplot(data = datos, aes(x = dolar, y = Unidades)) +
  ...
  geom_point(col="blue")+labs(title = "Gráfico correlación dólar vs Unidades registradas",caption = "Figura 4",
       x = "Precio del dólar",
...
- **observance:** Fechas especiales no festivas en Colombia
...
Se dividen los datos en train con los años de 2012 a 2017 y se testiza test con los datos del primer semestre del 2018. A continuación se lista el nombre de los festivos nacionales y las fechas especiales no festivas respectivamente:  
...
A continuación se puede observar un gráfico de cajas para la variable Unidades, donde se puede observar que la media del número de registros diarios se encuentra alrededor de 900[CORREGIR], además de una cantidad de registros atípicos, con valores entre los 2500 y más de 3500 en un día. Así entonces se procede a remover dichos valores de la base de entrenamiento.
...
test<- filter(datos2, datos2$year>2016)
```

# Modelos

[MEJORAR GRAFICOS CON GGPLOT]
...
par(mfrow=c(1,2))
plot(x = train$Unidades,mod1$fitted.values, 
     main = "Modelo Lineal Sin Outliers",
     col= "purple")
plot(x = train$Unidades,(mod2$fitted.values)^2, 
     main = "Modelo Raíz Sin Outliers",
     col= "orange")
...
Los modelos elegidos fueron: tal y tal. Así entonces, se vuelven a correr los modelos finales, con todos los datos desde el año 2012 hasta el año 2017 para realizar las predicciones al primer semestre del año 2018.
...
Las siguientes son las predicciones resultantes para el primer semestre del año 2018, realizadas por el modelo.
...
db2018 <- left_join(x = db2018, y = TRM[,c("Fecha","dolar")], 
                          by="Fecha")
pred3<- predict(mod_Final,newdata = db2018)
pred3 %>%kbl() %>%  kable_styling(font_size = 12)%>% kableExtra::scroll_box(width = "30%", height = "190px")
```
